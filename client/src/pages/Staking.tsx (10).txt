...
function StakedNFTCard({ tokenId, onUnstake, onClaim, onEmergency }: any) {
  const { data: stakeData, refetch: refetchStake } = useReadContract({
    address: STAKING_CONTRACT, abi: STAKING_ABI, functionName: 'stakes', args: [BigInt(tokenId)],
  });
  const { data: pendingRewards, refetch: refetchRewards } = useReadContract({
    address: STAKING_CONTRACT, abi: STAKING_ABI, functionName: 'calculateRewards', args: [BigInt(tokenId)],
  });
  const { data: rarityData } = useReadContract({
    address: STAKING_CONTRACT, abi: STAKING_ABI, functionName: 'tokenRarity', args: [BigInt(tokenId)],
  });
  ...
  useEffect(() => {
    const t = setInterval(() => { refetchStake(); refetchRewards(); }, 30000);
    return () => clearInterval(t);
  }, [refetchStake, refetchRewards]);
  ...
  const [, stakedAt, , unlockTime] = stakeData;
  ...
  const pendingFormatted = pendingRewards ? Math.floor(Number(formatEther(pendingRewards as bigint))) : 0;
      ...
      <main className="pt-32 pb-20 px-6 max-w-7xl mx-auto">
        {!isConnected ? (
          ...
          <>
            ...
            {/* Approve Banner */}
            {!isApproved && (
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center gap-4 justify-between mb-6">
                <div>
                  <p className="text-white font-semibold text-sm mb-0.5">Approve Staking Contract</p>
                  <p className="text-gray-500 text-xs">One-time approval required before staking any NFTs</p>
                </div>
                <button
                  onClick={() => writeContract({ address: SHACKO_NFT, abi: NFT_ABI, functionName: 'setApprovalForAll', args: [STAKING_CONTRACT, true] })}
                  disabled={isTxPending}
                  className="flex items-center gap-2 bg-[#fbbf24] hover:bg-[#f59e0b] disabled:opacity-50 text-black font-semibold px-5 py-2.5 rounded-xl transition-colors text-sm whitespace-nowrap"
                >
                  {isTxPending ? 'Approving...' : 'Approve Contract'} <ChevronRight size={14} />
                </button>
              </div>
            )}

            {/* Built on Base Banner */}
            <div className="relative overflow-hidden bg-gradient-to-r from-[#0d1525] to-[#0a1628] border border-blue-500/20 rounded-2xl p-6 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-6">
              ...
              <div className="flex-1 relative z-10">
                ...
                <p className="text-gray-400 text-sm mb-4">Earn $SHACK rewards by staking your SHACKO NFTs. All rewards are distributed on Base.</p>
              ...
              <div className="relative z-10 flex flex-wrap gap-2 flex-shrink-0">
                {stakedIds.length > 0 && (
                  <button
                    onClick={() => writeContract({ address: STAKING_CONTRACT, abi: STAKING_ABI, functionName: 'claimAllRewards', args: [] })}
                    disabled={isTxPending}
                    className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-semibold px-5 py-3 rounded-xl transition-colors text-sm"
                  >
                    {isTxPending ? 'Claiming...' : 'Claim All'}
                  </button>
                )}
                {unstakedNfts.length > 1 && isApproved && (
                  <button
                    onClick={() => setStakeModalToken(-1)}
                    disabled={isTxPending}
                    className="flex items-center gap-2 bg-[#fbbf24]/10 hover:bg-[#fbbf24]/20 border border-[#fbbf24]/30 text-[#fbbf24] font-semibold px-5 py-3 rounded-xl transition-colors text-sm disabled:opacity-50"
                  >
                    Stake All
                  </button>
                )}
                <a href="/rewards" className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold px-5 py-3 rounded-xl transition-colors text-sm">
            ...
            <div className="space-y-12">
              <section>
                <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Your Staked SHACKOs ({stakedIds.length})</h2>
              ...
              <section>
                <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Available to Stake ({unstakedNfts.length})</h2>
