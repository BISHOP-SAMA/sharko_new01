...
const STAKING_ABI = parseAbi([
  ...
  'struct StakeInfo { address owner; uint256 stakedAt; uint8 duration; uint256 unlockTime; uint256 lastClaimTime; bool isStaked; }',
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="relative overflow-hidden bg-gradient-to-r from-[#0d1525] to-[#0a1628] border border-blue-500/20 rounded-2xl p-6 mb-8 flex flex-col sm:flex-row items-center gap-6">
                ...
                <div className="flex-1 relative z-10">
                  ...
                  <h2 className="text-white font-bold text-lg mb-1">Stake NFTs &amp; Earn $SHACK</h2>
                  <p className="text-gray-400 text-sm mb-4">Earn $SHACK rewards by staking your SHACKO NFTs. All rewards distributed on Base.</p>
                ...
                <div className="relative z-10 flex-shrink-0 flex flex-col sm:flex-row gap-2">
                  {stakedNFTs && stakedNFTs.length > 0 && (
                    <button
                      ...
                      className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-semibold px-5 py-3 rounded-xl transition-colors text-sm"
                    >
                      {isClaimingAll ? 'Claiming...' : 'Claim All'}
                    </button>
                  )}
                  <a href="/rewards" className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold px-5 py-3 rounded-xl transition-colors text-sm">
              ...
              {stakedNFTs && stakedNFTs.length > 0 && (
                <div className="mb-10">
                  <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Staked SHACKOs</h2>
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">
                      Your SHACKOs ({unstakedTokenIds.length})
                    </h2>
                    {unstakedTokenIds.length > 1 && (
                      <button
                        onClick={() => setShowStakeAllModal(true)}
                        disabled={isStakingAll}
                        className="flex items-center gap-2 bg-[#fbbf24]/10 hover:bg-[#fbbf24]/20 border border-[#fbbf24]/30 text-[#fbbf24] font-semibold px-4 py-2 rounded-xl transition-colors text-sm disabled:opacity-50"
                      >
                        <Layers size={14} />
...
function UnstakedNFTCard({
  ...
  const { data: rarity } = useReadContract({
    address: STAKING_CONTRACT, abi: STAKING_ABI, functionName: 'tokenRarity',
    args: [BigInt(tokenId)],
  });
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className="bg-[#0d1525] border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-colors flex flex-col"
    >
      {/* NFT Visual Placeholder */}
      <div className="w-full aspect-square rounded-xl bg-gradient-to-br from-white/5 to-white/[0.02] border border-white/5 flex items-center justify-center mb-4">
        <p className="text-4xl font-black text-white/10" style={{ fontFamily: 'Impact, sans-serif' }}>
          #{tokenId}
        </p>
      </div>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`px-2 py-0.5 rounded-md text-xs font-medium border ${rCol.bg} ${rCol.text} ${rCol.border}`}>
          {r}
        </span>
      </div>

      <div className="bg-white/5 rounded-xl p-3 mb-4 text-center">
        <p className="text-xs text-gray-500 mb-0.5">Daily Earnings</p>
        <p className="text-sm font-bold text-[#fbbf24]">
          {baseRateMap[r] ?? 10} $SHACK/day
        </p>
      </div>
      ...
      <button
        ...
        disabled={isStaking}
        className="w-full bg-[#fbbf24] hover:bg-[#f59e0b] disabled:opacity-50 text-black font-semibold py-2.5 rounded-xl transition-colors text-sm mt-auto"
      >
...
function StakedNFTCard({
  ...
  const durInfo = { ...{ label: '7 Days', multiplier: '1.0x', mult: 100 }, ...([
    { label: '7 Days', multiplier: '1.0x', mult: 100 },
    { label: '14 Days', multiplier: '1.25x', mult: 125 },
    { label: '30 Days', multiplier: '1.5x', mult: 150 },
    { label: '60 Days', multiplier: '2.0x', mult: 200 },
  ][durIndex]) };
  ...
  return (
    ...
    >
      {/* NFT Visual */}
      <div className="w-full aspect-square rounded-xl bg-gradient-to-br from-white/5 to-white/[0.02] border border-white/5 flex items-center justify-center mb-4 relative overflow-hidden">
        <p className="text-4xl font-black text-white/10" style={{ fontFamily: 'Impact, sans-serif' }}>
          #{tokenId}
        </p>
        {/* Staked badge */}
        <div className="absolute top-2 right-2 flex items-center gap-1 bg-emerald-500/20 border border-emerald-500/30 rounded-lg px-2 py-1">
          <Lock size={10} className="text-emerald-400" />
          <span className="text-xs text-emerald-400 font-semibold">Staked</span>
        </div>
      </div>
      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="text-white font-bold text-sm">SHACKO #{tokenId}</p>
          <span className={`inline-block mt-1 px-2 py-0.5 rounded-md text-xs font-medium border ${rCol.bg} ${rCol.text} ${rCol.border}`}>
            {rarity}
          </span>
        </div>
        <div className="text-right">
          <p className="text-xs text-gray-600 mb-0.5">Pending</p>
          <p className="text-base font-bold text-emerald-400">{pendingFormatted.toLocaleString()}</p>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="bg-white/5 rounded-xl p-2 text-center">
          ...
          <p className="text-xs font-semibold text-white">{durInfo.label}</p>
          <p className="text-xs text-[#fbbf24]">{durInfo.multiplier}</p>
        </div>
        <div className="bg-white/5 rounded-xl p-2 text-center">
          <p className="text-xs text-gray-600 mb-0.5">Per Day</p>
          <p className="text-xs font-semibold text-white">{earningsPerDay}</p>
          <p className="text-xs text-gray-600">$SHACK</p>
        </div>
        <div className={`rounded-xl p-2 text-center ${isUnlocked ? 'bg-emerald-500/10 border border-emerald-500/20' : 'bg-white/5'}`}>
          <p className="text-xs text-gray-600 mb-0.5">Time Left</p>
          {isUnlocked ? (
            <p className="text-xs font-semibold text-emerald-400">Ready!</p>
          ) : (
            <p className="text-xs font-semibold text-white tabular-nums">{formatCountdown(secondsLeft)}</p>
          )}
        </div>
      </div>
      ...
      {!isUnlocked && (
        <button
          onClick={() => {
            if (confirm('⚠️ Emergency unstake will forfeit ALL your pending $SHACK rewards and cost 0.0002 ETH. Are you sure?')) {
