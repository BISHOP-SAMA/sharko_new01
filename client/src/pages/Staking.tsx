...
const STAKING_ABI = parseAbi([
  ...
  'function baseRates(string rarity) view returns (uint256)',
  'function getRarityMultiplier(string rarity) view returns (uint256)',
  'function getDurationMultiplier(uint8 duration) view returns (uint256)',
            ...
            >
              ...
              <div className="mb-8">
                ...
                <input
                  ...
                  className="w-full h-16 rounded-xl border-4 border-black bg-white px-6 text-xl font-bold text-black placeholder-gray-400"
                />
              </div>
              ...
              <div className="bg-gradient-to-r from-[#fbbf24]/20 to-[#f59e0b]/20 border-2 border-[#fbbf24] rounded-2xl p-6 mb-8 text-center">
                <p className="text-xl font-bold text-white">
                  Stake Fee: 0.0001 ETH (~$0.0005 on Base)
                </p>
              </div>
...
function StakedNFTCard({ 
  ...
  // Main stake info (owner, stakedAt, unlockTime, pendingRewards, isStaked, rarity)
  ...
  // Raw stake struct to get duration enum value
  const { data: rawStake } = useReadContract({
    address: STAKING_CONTRACT,
    abi: STAKING_ABI,
    functionName: 'stakes',
    args: [BigInt(tokenId)],
  ...
  if (!stakeInfo || !rawStake) return null;

  const [, stakedAt, unlockTime, pendingRewards, , rarity] = stakeInfo;
  const [, , durationEnum] = rawStake;

  const now = Date.now() / 1000;
  const isUnlocked = Number(unlockTime) <= now;
  const daysLeft = Math.max(0, Math.ceil((Number(unlockTime) - now) / 86400));

  // Duration label & multiplier derived from enum (0=7d, 1=14d, 2=30d, 3=60d)
  const durationMap: Record<number, { label: string; days: number; multiplier: string; mult: number }> = {
    0: { label: '7 Days',  days: 7,  multiplier: '1.0x', mult: 100 },
    1: { label: '14 Days', days: 14, multiplier: '1.25x', mult: 125 },
    2: { label: '30 Days', days: 30, multiplier: '1.5x', mult: 150 },
    3: { label: '60 Days', days: 60, multiplier: '2.0x', mult: 200 },
  };
  const durationInfo = durationMap[Number(durationEnum)] ?? durationMap[0];

  // Base rates per rarity (mirrors contract values exactly)
  ...
  // Rarity multiplier (mirrors contract getRarityMultiplier)
  ...
  // earningsPerDay = baseRate × rarityMult/100 × durationMult/100
  const earningsPerDay = (baseRate * rarityMult * durationInfo.mult) / (100 * 100);
    ...
    <div className="bg-gradient-to-br from-[#1a3a52] to-[#0f1729] border-4 border-black rounded-3xl p-6 shadow-2xl">
      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          {/* Rarity Badge */}
          <span className={`inline-block px-4 py-1.5 rounded-xl text-sm font-bold text-white bg-gradient-to-r ${rarityColors[rarity] || rarityColors['Common']} border-2 border-black`}>
            {rarity}
          </span>
        </div>
        <div className="text-right bg-white/10 backdrop-blur-sm rounded-2xl p-4 border-2 border-white/20">
          ...
          <p className="text-xs text-gray-400 font-semibold">xSHACK</p>
        </div>
      </div>

      {/* Stats Row: Duration | Earnings/day */}
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-3 border-2 border-white/20 text-center">
          <p className="text-xs text-gray-400 font-semibold mb-1">Duration</p>
          <p className="text-lg font-black text-white" style={{ fontFamily: "'Bebas Neue', 'Impact', sans-serif" }}>
            {durationInfo.label}
          </p>
          <p className="text-xs text-[#fbbf24] font-bold">{durationInfo.multiplier} Bonus</p>
        </div>
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-3 border-2 border-white/20 text-center">
          <p className="text-xs text-gray-400 font-semibold mb-1">Earnings / Day</p>
          <p className="text-lg font-black text-[#10b981]" style={{ fontFamily: "'Bebas Neue', 'Impact', sans-serif" }}>
            {earningsPerDay.toLocaleString()}
          </p>
          <p className="text-xs text-gray-400 font-bold">xSHACK</p>
        </div>
      </div>
      ...
      <div className="mb-5">
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-sm text-gray-300 font-bold">
            {isUnlocked ? '✅ Unlocked — ready to unstake' : `${daysLeft} day${daysLeft !== 1 ? 's' : ''} remaining`}
          </p>
        </div>
      ...
      {/* Emergency Unstake — only shown while locked */}
      {!isUnlocked && (
        <button
          onClick={() => {
          ...
          className="w-full mt-3 bg-gradient-to-r from-[#ef4444] to-[#b91c1c] text-white px-4 py-3 rounded-xl font-black border-4 border-black hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed text-sm shadow-lg"
          style={{ fontFamily: "'Bebas Neue', 'Impact', sans-serif" }}
        >
          {isEmergencyUnstaking ? 'PROCESSING...' : '⚡ EMERGENCY UNSTAKE (0.0002 ETH) — FORFEITS ALL REWARDS'}
        </button>
      )}
    </div>
  );
