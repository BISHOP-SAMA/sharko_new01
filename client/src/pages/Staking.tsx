            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="relative overflow-hidden bg-gradient-to-r from-[#0d1525] to-[#0a1628] border border-blue-500/20 rounded-2xl p-6 mb-8 flex flex-col sm:flex-row items-center gap-6">
                ...
                <div className="absolute right-0 top-0 w-48 h-48 opacity-10 pointer-events-none">
                  <img src="/Base.webp" alt="" className="w-full h-full object-cover" />
                </div>
                {/* Left: text */}
                <div className="flex-1 relative z-10">
                  <div className="flex items-center gap-2 mb-2">
                    <img src="/Base.webp" alt="Base" className="w-5 h-5 rounded-full" />
...
function StakedNFTCard({
  ...
  const { data: rawStake } = useReadContract({
    address: STAKING_CONTRACT, abi: STAKING_ABI, functionName: 'stakes',
    args: [BigInt(tokenId)],
  });
  ...
  const durationEnum = rawStake ? rawStake[2] : 0;
  ...
  const durInfo = durationMap[Number(durationEnum)] ?? durationMap[0];
