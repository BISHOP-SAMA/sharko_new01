...
function StakedNFTCard({ 
  ...
  return (
    <div className="bg-gradient-to-br from-[#1a3a52] to-[#0f1729] border-4 border-black rounded-3xl p-6 shadow-2xl">
      ...
      {!isUnlocked && (
        <button
          onClick={() => onEmergencyUnstake(tokenId)}
          disabled={isEmergencyUnstaking}
          className="w-full mt-3 bg-gradient-to-r from-[#ef4444] to-[#b91c1c] text-white px-4 py-3 rounded-xl font-black border-4 border-black hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed text-base shadow-lg"
          style={{ fontFamily: "'Bebas Neue', 'Impact', sans-serif" }}
          title="Emergency unstake costs 0.0002 ETH fee"
        >
          {isEmergencyUnstaking ? 'PROCESSING...' : '⚡ EMERGENCY UNSTAKE (0.0002 ETH)'}
        </button>
      )}
    </div>
  );
